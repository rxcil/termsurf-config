...
local config = {
  ...
  keys = {
    -- Close the current pane (with confirmation prompt to avoid accidents)
    {
      ...
      action = wezboard.action.CloseCurrentPane({ confirm = true }),
    },

    -- New tab: cmd+t
    { key = "t", mods = "CMD", action = wezboard.action({ SpawnTab = "DefaultDomain" }) },

    -- Move right one tab: ctrl+shift+r
    { key = "l", mods = "CTRL|SHIFT", action = wezboard.action({ ActivateTabRelative = 1 }) },

    -- Move left one tab: ctrl+shift+l
    { key = "h", mods = "CTRL|SHIFT", action = wezboard.action({ ActivateTabRelative = -1 }) },

    -- Split left shortcut
    {
      key = ";",
      mods = "CMD",
      action = wezboard.action.SplitPane({
        direction = "Left",
        size = { Cells = 100 },
      }),
    },
    ...
    { key = "j", mods = "CMD", action = wezboard.action.SplitPane({ direction = "Down" }) },

    -- Split vertical up: cmd+k
    { key = "k", mods = "CMD", action = wezboard.action.SplitPane({ direction = "Up" }) },

    -- Split horizontal right: cmd+l
    { key = "l", mods = "CMD", action = wezboard.action.SplitPane({ direction = "Right" }) },

    -- Split horizontal left: cmd+h
    { key = "h", mods = "CMD", action = wezboard.action.SplitPane({ direction = "Left" }) },

    -- Move right one window (rotate right): ctrl+r
    { key = "l", mods = "CTRL", action = wezboard.action({ ActivatePaneDirection = "Right" }) },

    -- Move left one window (rotate left): ctrl+l
    { key = "h", mods = "CTRL", action = wezboard.action({ ActivatePaneDirection = "Left" }) },

    -- Move up one window (rotate up): ctrl+k
    { key = "k", mods = "CTRL", action = wezboard.action({ ActivatePaneDirection = "Up" }) },

    -- Move down one window (rotate down): ctrl+j
    { key = "j", mods = "CTRL", action = wezboard.action({ ActivatePaneDirection = "Down" }) },

    -- Resize the current pane left: ctrl+alt+h
    {
      key = "LeftArrow",
      mods = "CTRL|ALT",
      action = wezboard.action.AdjustPaneSize({ "Left", 10 }),
    },
    ...
    -- Resize the current pane right: ctrl+alt+l
    {
      key = "RightArrow",
      mods = "CTRL|ALT",
      action = wezboard.action.AdjustPaneSize({ "Right", 10 }),
    },
    ...
    -- Resize the current pane up: ctrl+alt+k
    {
      key = "UpArrow",
      mods = "CTRL|ALT",
      action = wezboard.action.AdjustPaneSize({ "Up", 10 }),
    },
    ...
    -- Resize the current pane down: ctrl+alt+j
    {
      key = "DownArrow",
      mods = "CTRL|ALT",
      action = wezboard.action.AdjustPaneSize({ "Down", 10 }),
    },
    ...
    -- Rename tab: ctrl+shift+t
    {
      ...
      action = wezboard.action.PromptInputLine({
        ...
        action = wezboard.action_callback(function(window, pane, line)
    ...
    -- Move tab to the left: ctrl+shift+left
    {
      key = "LeftArrow",
      mods = "CTRL|SHIFT",
      action = wezboard.action.MoveTabRelative(-1),
    },
    ...
    -- Zoom: focus on the current pane and hide the others
    {
      key = "Z",
      mods = "CTRL|SHIFT",
      action = wezboard.action.TogglePaneZoomState,
    },
  },

  font_size = 11,
  font = wezboard.font("JetBrainsMono Nerd Font"),
  ...
  inactive_pane_hsb = {
    ...
    -- saturation = 0.5, -- Reduce saturation for inactive panes
    brightness = 1.0, -- Make inactive panes slightly dimmer
  },
  ...
  initial_cols = 160, -- number of character cells wide
  ...
  mouse_bindings = {
    -- disable default left-click behavior (so plain click won’t open link)
    {
      event = { Up = { streak = 1, button = "Left" } },
      mods = "NONE",
      action = wezboard.action.DisableDefaultAssignment,
    },
    -- require Ctrl+click to open link
    {
      event = { Up = { streak = 1, button = "Left" } },
      mods = "CTRL",
      action = wezboard.action.OpenLinkAtMouseCursor,
    },
    -- disable Ctrl+click down event to avoid weird behaviors
    {
      event = { Down = { streak = 1, button = "Left" } },
      mods = "CTRL",
      action = wezboard.action.Nop,
    },
  },
}
